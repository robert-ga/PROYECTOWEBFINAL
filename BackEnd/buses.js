...
router.post('/', async(req, res) => {
    const { origenbus, destinobus, dia, hora, nombrebus, tipobus, precio } = req.body
    let sql = `INSERT INTO buses(origenbus, destinobus,dia,hora,nombrebus,tipobus,precio) values('${origenbus}',
    '${destinobus}','${dia}','${hora}','${nombrebus}','${tipobus}','${precio}')`
    conexion.query(sql, (err, rows, fields) => {
        if (err) {
